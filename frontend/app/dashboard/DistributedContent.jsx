  ...
  useEffect(() => {
    const fetchDistributedData = async () => {
      try {
        ...
        const response = await fetch('/api/distributed-data');
        const data = await response.json();
        
        if (data.distributedData && data.csvData) {
          setDistributedData(data.distributedData);
                    ...
                    {item.data.map((row, rowIndex) => (
                      <tr key={rowIndex} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        {Object.values(row).map((value, colIndex) => (
                          <td key={colIndex} className="py-3 px-6 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-300 dark:border-gray-700">
                            {value}
                          </td>
                        ))}
                      </tr>
